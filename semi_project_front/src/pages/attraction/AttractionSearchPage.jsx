...
const AttractionSearchPage = ({ categoryTest, setCategoryTest }) => {
  ...
  const isFromWrite = location.state?.fromWrite;
  ...
  const [reviewCategory, setReviewCategory] = useState(0); // 0: 추천(좋아요순), 1: 최신순, 2: 오래된순
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKSERVER}/attractions/addressList`, {
        params: {
          areaCode: areaCode,
  ...
  }, [searchKeyword, fee, restroom, accessible, parking, checkedItems]);
    ...
    <>
      <section className={styles.attraction_search_wrap}>
        <div className={styles.menubar}>
          <div className={styles.select_where} ref={whereRef}>
            <div
              className={styles.select_where_title}
              onClick={(e) => {
                e.stopPropagation();
                setIsWhereOpen((prev) => !prev);
              }}
            >
              <div>지역 선택</div>
              {isWhereOpen ? (
        ...
        <div className={styles.content}>
          <Map
            ...
            currentList={attractionList.map((item) => item.addr)}
          ></Map>
                ...
                attractionList.map((item, index) => {
                  ...
                  return (
                    <AttractionList
                      ...
                      isFromWrite={isFromWrite} // 현재 페이지가 게시글 작성에서 온건지 확인
                      selectAttraction={() => {
                        if (isFromWrite) {
                          navigate("/board/write", {
                            state: {
                              selectedPlace: item.title,
                              placeName: item.title, //게시글 상세보기
                              fromAttraction: true,
                              attractionNo: item.attractionNo, //게시글 DB에 저장할 관광지 번호
                              prevBoard: location.state?.prevBoard, //넘어왔던 작성 중인 데이터를 다시 전달하여 보존
                            },
                          });
                        } else {
                          setCategoryTest(0);
